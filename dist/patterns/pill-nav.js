...
export function PillNavLink({ href, children, className, active = false, asChild = false, }) {
    const Comp = asChild ? Slot : 'a';
    return (_jsx(Comp, { ...(asChild ? {} : { href }), "data-pill-nav-link": "", className: cn('rounded-[4px] px-4 py-1.5 text-sm tracking-[-0.01em] whitespace-nowrap shrink-0 transition-colors duration-150', active ? 'text-brand-peach' : 'text-white/90', 'hover:bg-white/[0.06] hover:text-chrome-text-primary', className), children: children }));
}
