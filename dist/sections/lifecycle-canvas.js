...
// `DiscoveryTable`.
import { artifactHeader, AppWindow } from './artifact-shell.js';
...
/* ── LifecycleCanvas ──────────────────────────────────────────────────────── */
export function LifecycleCanvas({ stages, title, source, separator = '/', summary, summaryStatus = 'ok', actions, className, }) {
    ...
    return (_jsx(AppWindow, { crumb: crumb, actions: bar, className: className, children: _jsx("ol", { className: "m-0 grid list-none grid-flow-col auto-cols-[minmax(190px,1fr)] gap-[12px] overflow-x-auto p-0", children: stages.map((stage, i) => (
            ...
            _jsxs("li", { className: "flex min-w-0 flex-col gap-[8px]", children: [_jsxs("div", { className: "rounded-sm border border-border bg-muted p-[12px]", children: [stage.key ? (_jsx("span", { className: "mb-[4px] block font-mono text-[9px] uppercase tracking-[0.9px] text-muted-foreground [overflow-wrap:anywhere]", children: stage.key })) : null, _jsx("strong", { className: "block text-[16px] font-medium text-foreground", children: stage.name }), stage.description ? (_jsx("small", { className: "mt-[2px] block text-[12px] text-muted-foreground", children: stage.description })) : null] }), stage.milestones?.map((milestone, j) => (
                    ...
                    _jsxs("div", { className: "flex-auto rounded-sm border border-border bg-card p-[12px]", children: [_jsx("strong", { className: "block text-[13px] font-medium text-foreground", children: milestone.name }), milestone.description ? (_jsx("p", { className: "mt-[4px] text-[12px] text-muted-foreground", children: milestone.description })) : null, milestone.bindings?.length ? (
                            ...
                            _jsx("div", { className: "mt-[8px]", children: milestone.bindings.map((binding, k) => (_jsx(BindingTag, { children: binding }, k))) })) : null] }, j)))] }, i))) }) }));
}
