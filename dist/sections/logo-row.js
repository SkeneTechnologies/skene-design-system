...
export function LogoSlot({ children, label, className }) {
    return (_jsx("div", { "aria-label": children ? label : undefined, 
        ...
        // nothing.
        className: cn('grid min-h-14 place-items-center rounded-[var(--radius-md)] border border-border bg-muted', className), children: children }));
}
export function LogoRow({ title, stat, count = 5, children, caption, decorative = true, className, }) {
    ...
    return (_jsxs("section", { className: cn('mx-auto w-full', className), children: [title ? (_jsx("div", { className: "mx-auto mb-6 max-w-[640px] text-center", children: _jsx("h2", { className: "text-[clamp(1.4rem,2.2vw,1.8rem)] font-medium tracking-[-0.01em] text-text-primary", children: title }) })) : null, stat ? (
            ...
            _jsx("p", { className: "mx-auto mb-6 max-w-[560px] text-center text-[14px] leading-relaxed text-text-muted-strong [&_strong]:font-medium [&_strong]:text-text-primary", children: stat })) : null, _jsxs("div", { "aria-hidden": decorative || undefined, className: "mx-auto grid max-w-[900px] grid-cols-2 gap-3.5 sm:grid-cols-[repeat(var(--logo-row-count),1fr)]", style: { '--logo-row-count': count }, children: [children, Array.from({ length: blanks }, (_, i) => (_jsx(LogoSlot, {}, i)))] }), caption ? (_jsx("p", { className: "mx-auto mt-3.5 max-w-[480px] text-center text-[12.5px] leading-[1.6] text-text-muted", children: caption })) : null] }));
}
