...
export function QuestionGrid({ columns = 3, className, children }) {
    return (
    ...
    _jsx("div", { className: cn('grid gap-4', COLUMN_CLASS[columns], className), children: children }));
}
export function QuestionCard({ tag, title, children, className }) {
    return (_jsxs("article", { className: cn('flex h-full flex-col rounded-[14px] border border-surface-border p-7', 
        ...
        'min-h-[230px] md:min-h-[270px]', className), style: {
            // See the file header: a corner glow, not a wash. The stop reaches
            // transparent well before the far corner.
            backgroundImage: 'radial-gradient(78% 72% at 100% 0%, color-mix(in oklab, var(--color-brand-peach) 9%, transparent) 0%, transparent 66%)',
        }, children: [tag ? (
            // The 58px is the point of the card. See the file header.
            _jsx("span", { className: "mb-[58px] font-mono uppercase text-brand-peach", style: {
                    fontSize: 'var(--font-size-pill)',
                    letterSpacing: 'var(--font-tracking-eyebrow)',
                }, children: tag })) : null, _jsx("h3", { className: "text-[1.45rem] font-normal leading-[1.2] tracking-[-0.02em] text-text-primary", children: title }), children ? (_jsx("p", { className: "mt-3.5 text-[0.9rem] leading-relaxed text-text-muted", children: children })) : null] }));
}
