...
| A hub whose cards are colour-coded by topic. The accent is a named union over tokens this package already ships, six neon and gold roles plus the brand peach. | `prop HubCard.accent` |
...
| note | via |
| --- | --- |
| accent is a NAMED union, not a hex, and that is the point. The implementation this was extracted from took a free-form hex and one of its call sites passed the literal #fac089 against a brand peach of #fec089. One character, shipped, invisible to every gate because a raw hex in a styled-components prop is not a Tailwind arbitrary value. A union will not compile the typo. | `prop HubCard.accent` |
