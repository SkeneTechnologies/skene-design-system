...
import { AppPanel, AppWindow, type StatPillStatus, PanelCaption } from './artifact-shell.js'
 ...
 */
export function CheckChip({ role = false, className, children }: CheckChipProps) {
  return (
    <Badge
      variant="outline"
      className={cn(
        'max-w-full rounded-sm px-[8px] py-[1px] font-mono text-[11px] font-normal wrap-anywhere',
        role ? 'border-foreground bg-foreground text-background' : 'bg-muted',
        className,
      )}
    >
      {children}
 ...
 */
const DOT_TOKEN: Record<StatPillStatus, string> = {
 ...
 * figure is data.
 */
export function CheckFigure({ label, value, className }: CheckFigureProps) {
  return (
    <div className={cn('text-right', className)}>
      <span className="block font-sans text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
        {label}
      </span>
      <b className="font-mono text-[14px] font-medium tabular-nums text-foreground">{value}</b>
    </div>
  )
}
...
export function CheckOperand({
  ...
  return (
    <div
      className={cn(
        'flex flex-wrap items-center justify-between gap-[12px] border-b p-[12px]',
        className,
      )}
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-[8px]">
        <span
          ...
          style={{ background: DOT_TOKEN[tone] }}
        />
        <span className="font-mono text-[13px] text-foreground wrap-anywhere">{name}</span>
        {chips}
      </div>
      {figures ? <div className="flex gap-[24px]">{figures}</div> : null}
    </div>
  )
}
 ...
 */
export function CheckResult({ label, value, className }: CheckResultProps) {
  return (
    <div className={cn('min-w-0 rounded-sm border border-border bg-card p-[12px]', className)}>
      <span className="block text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
        {label}
      </span>
      <b className="mt-[8px] block text-[14px] font-medium text-foreground">{value}</b>
    </div>
  )
}
...
export function EvaluatorCheck({
  ...
  return (
    <AppWindow crumb={crumb} actions={actions} className={className}>
      <AppPanel>
        <PanelCaption>
          <span>{heading}</span>
          {headingNote ? (
            <span className="text-[11px] text-muted-foreground">{headingNote}</span>
          ) : null}
        </PanelCaption>

        <div className="flex flex-wrap items-start justify-between gap-[12px] border-b border-border p-[12px]">
          <div className="min-w-0">
            <div className="text-[14px] text-foreground">{metric}</div>
            {formula ? (
              <div className="mt-[2px] font-mono text-[12px] text-muted-foreground wrap-anywhere">
                {formula}
              </div>
            ) : null}
          </div>
          {qualifiers ? <div className="flex flex-wrap gap-[8px]">{qualifiers}</div> : null}
        </div>

        {children}

        {results ? (
          <div className="grid grid-cols-[repeat(auto-fit,minmax(min(140px,100%),1fr))] gap-[12px] p-[12px]">
            {results}
          </div>
        ) : null}
      </AppPanel>

      {/* NOT `EvaluatorNote`, and the difference is a real one: this copy has no
          `[&_code]` rules, so a <code> in the note renders as body text here and
          as mono foreground everywhere else. Adopting the shared part would
          therefore change pixels, which is why it was left out of the
          zero-diff merge on 2026-08-13 rather than swept into it. Fix it as a
          deliberate change with its own baseline movement. */}
      {note ? <div className="mt-[12px] text-[12px] text-muted-foreground">{note}</div> : null}
    </AppWindow>
  )
}
