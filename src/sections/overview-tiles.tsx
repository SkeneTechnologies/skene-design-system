 ...
 */
export function OverviewTiles({ children, className }: OverviewTilesProps) {
  return (
    <div
      className={cn(
        'grid gap-[12px] [grid-template-columns:repeat(auto-fit,minmax(min(160px,100%),1fr))]',
        className,
      )}
    >
      {children}
 ...
 */
export function OverviewTile({ label, value, note, className }: OverviewTileProps) {
  return (
    <div
      className={cn('min-w-0 rounded-sm border border-border bg-card p-[12px]', className)}
    >
      <span className="mb-[8px] block font-mono text-[9px] uppercase tracking-[0.9px] text-muted-foreground">
        {label}
      </span>
      <div className="break-words text-[24px] leading-[1.15] text-foreground">{value}</div>
      {note ? (
        <small className="mt-[4px] block text-[12px] text-muted-foreground">{note}</small>
      ) : null}
    </div>
  )
}
