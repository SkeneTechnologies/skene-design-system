...
export function DiscoveryTable({
  ...
  return (
    <AppWindow crumb={crumb} actions={bar} className={className}>
      {/* The panel is the scroll container's outer clip: at 390px these four
          columns are wider than the viewport and the table scrolls sideways
          inside the artifact instead of widening the page. Never put a width on
          the panel — see AppPanel. */}
      <AppPanel>
        <DataTable columns={[columns.event, columns.type, columns.foundAt, columns.status]}>
          {rows.map((row, i) => (
            <DataRow key={i}>
              <DataCell mono>{row.event}</DataCell>
              <DataCell>{row.type}</DataCell>
              <DataCell mono muted>
                {row.foundAt}
              </DataCell>
              <DataCell>
                <StatPill status={row.status}>{row.statusLabel}</StatPill>
              </DataCell>
            </DataRow>
          ))}
        </DataTable>
