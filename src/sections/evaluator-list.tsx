...
export interface EvaluationEntry {
  ...
  check: { status: StatPillStatus; label: React.ReactNode }
...
export function EvaluatorList({
  ...
  const artifact = (
    ...
    >
      {/*
        No top margin. The prototype's `.evl` carries one for a second panel and
        then cancels it again for the first child of the body — which this always
        is, because an artifact shows one index. Two rules that net to nothing.
        ...
        the artifact stays off the page's own horizontal scrollbar. Do not add
        one here to make the columns "look right" on a phone — that moves the
        scrollbar from the panel to the page, which is the gate this artifact set
        is verified against.
      */}
      <AppPanel>
        <DataTable columns={[columns.name, columns.check, columns.metric, columns.confirmed]}>
 ...
 */
export function EvaluatorNote({ children, className }: EvaluatorNoteProps) {
  return (
    <div
      className={cn(
        ...
        'mt-[12px] text-[12px] text-muted-foreground [&_code]:font-mono [&_code]:text-foreground [&_code]:wrap-anywhere',
        className,
      )}
    >
      {children}
