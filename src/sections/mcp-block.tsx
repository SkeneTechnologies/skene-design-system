 ...
 */
export function McpBlock({ title, meta, children, className }: McpBlockProps) {
  return (
    <AppPanel className={cn('[&:not(:first-child)]:mt-[12px]', className)}>
      {title || meta ? (
        <PanelCaption>
          <span className="min-w-0">{title}</span>
          {meta ? (
            <span className="min-w-0 text-[11px] text-muted-foreground">{meta}</span>
          ) : null}
        </PanelCaption>
      ) : null}
      {children}
 ...
 */
export function McpCode({ children, className }: McpCodeProps) {
  return (
    <pre
      className={cn(
        'm-0 overflow-x-auto whitespace-pre p-[12px] font-mono text-[12px] leading-normal text-foreground',
        className,
      )}
    >
      {children}
 ...
 */
export function McpTool({ name, description, className }: McpToolProps) {
  return (
    <div className={cn('border-b border-border p-[12px] last:border-b-0', className)}>
      <b className="block font-mono text-[13px] font-normal text-foreground wrap-anywhere">
        {name}
      </b>
      {description ? (
        <span className="mt-[2px] block text-[12px] text-muted-foreground">{description}</span>
      ) : null}
    </div>
  )
}
