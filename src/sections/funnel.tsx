...
const STATE_COLOR: Record<FunnelStepState, string> = {
 ...
 */
export function FunnelRow({ label, note, value, state, fill, className }: FunnelRowProps) {
  const unknown = state === 'unknown'
  return (
    <div
      className={cn(
        'grid min-w-0 grid-cols-[minmax(0,1fr)] items-center gap-[8px]',
        'sm:grid-cols-[180px_minmax(0,1fr)_84px] sm:gap-[16px]',
        className,
      )}
    >
      <div className="min-w-0 text-[13px] text-foreground">
        {label}
        {note ? (
          <small className="mt-[2px] block font-mono text-[11px] text-muted-foreground">
            {note}
          </small>
        ) : null}
      </div>
      ...
      <div
        aria-hidden
        className={cn(
          'h-[24px] min-w-0 overflow-hidden rounded-sm border',
          unknown ? 'border-dashed border-muted-foreground bg-transparent' : 'border-border bg-muted',
        )}
      >
        {!unknown && fill !== undefined ? (
          <div
      ...
      <div
        className={cn(
          'font-mono text-[12px] tabular-nums text-left sm:text-right',
...
export function Funnel({
  ...
  return (
    <AppWindow className={className}>
      <AppPanel>
        <div className="flex flex-wrap items-center gap-[12px] border-b border-border px-[16px] py-[12px] text-[14px] text-foreground">
          <span
            aria-hidden
            className="size-[8px] shrink-0 rounded-full"
            style={{ background: STATE_COLOR[status] }}
          />
          <span>{title}</span>
          {badge}
          {meta ? (
        ...
        <div className="flex min-w-0 flex-col gap-[12px] p-[16px]">{children}</div>
      </AppPanel>
    </AppWindow>
  )
}
