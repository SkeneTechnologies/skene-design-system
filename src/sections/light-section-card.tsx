...
export function LightSectionCard({
  ...
  reverse = false,
  className,
}: LightSectionCardProps) {
  const hasProof = Boolean(children || actions)
  return (
    <section
      className={cn(
        // `light` first, and never conditional — see the file header.
        'light grid overflow-hidden rounded-3xl border border-chrome-line-on-light bg-brand-light',
        visual && (reverse ? 'md:grid-cols-[0.9fr_1.1fr]' : 'md:grid-cols-[1.1fr_0.9fr]'),
        className,
      )}
    >
      <div
        className={cn(
          'flex flex-col items-start px-8 pb-[46px] pt-[50px] md:px-12',
      ...
      >
        ...
        {lede ? (
          <p className="mt-4 max-w-[470px] text-[15px] italic text-text-muted">{lede}</p>
        ) : null}

        {hasProof ? (
          <div
            className="mt-7 w-full border-t pt-7"
            style={{ borderTopColor: 'var(--color-chrome-line-on-light)' }}
          >
            {children ? (
              ...
              <div className="w-full text-[15px] leading-relaxed text-text-muted-strong">
                {children}
              </div>
            ) : null}
            {actions ? (
              <div className={cn('flex flex-wrap items-center gap-3', children && 'mt-7')}>
                {actions}
              </div>
            ) : null}
          </div>
