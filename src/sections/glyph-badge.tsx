 ...
 */
const TONES: Record<GlyphBadgeTone, { className?: string; style?: React.CSSProperties }> = {
  tint: {
...
export function GlyphBadge({
  ...
  return (
    <span
      ...
      className={cn(
        'grid shrink-0 place-items-center rounded-full border text-brand-peach',
        toneClassName,
        className,
      )}
      style={{ width: size, height: size, fontSize: glyphSize, ...style }}
    >
      {children}
    </span>
  )
